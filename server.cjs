...
app.post('/api/submit-data', async (req, res) => {
  try {
    const { 
      ...
      zeroCircleFlow, 
      machineTotalFlow, 
      dayManualInjection = 2000, 
      dayInjectionConcentration = "艾烤糊精", 
      dayUltrafiltration = 0, 
      waterIntake,
      date,
      excelPath
    } = req.body;
    ...
    // 计算机器+手工总超滤量
    const machinePlusManualFlow = Number(machineTotalFlow) + Number(dayUltrafiltration);
    ...
    let savePath;
    if (excelPath) {
      // 标准化路径输入（去除前后空格，转为小写进行比较）
      const normalizedExcelPath = excelPath.trim().toLowerCase();
      
      console.log(`标准化后的excelPath: "${normalizedExcelPath}"`);
      
      // 如果提供了路径，确保它是绝对路径
      if (excelPath.includes(':')) {
        // 处理完整的绝对路径（如 "C:\Users\..."）
        savePath = excelPath;
      } else if (excelPath.startsWith('\\')) {
        // 处理网络路径或绝对路径
        savePath = excelPath;
      } else if (normalizedExcelPath === 'desktop') {
        // 特殊处理桌面路径
        savePath = path.join(process.env.USERPROFILE || process.env.HOME || __dirname, 'Desktop');
        console.log(`使用桌面路径: ${savePath}`);
      } else if (normalizedExcelPath === 'downloads') {
        // 特殊处理下载路径
        savePath = path.join(process.env.USERPROFILE || process.env.HOME || __dirname, 'Downloads');
        console.log(`使用下载路径: ${savePath}`);
      } else {
        // 处理相对路径，默认为当前目录
        savePath = path.join(__dirname, excelPath);
        console.log(`使用相对路径: ${savePath}`);
      }
    } else {
      // 默认保存到当前目录
      savePath = __dirname;
